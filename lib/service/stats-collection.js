...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'stai-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('stai').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('stai')[service]);
